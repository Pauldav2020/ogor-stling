...
$fName = $type = $dob = $gen = $marital = $contry = $city = $zip
  = $ssn = $phone = $email = $occup = $kin = $pass
  = $confirm = $user = $curr = $reg_Ref = $day = $month = $year = '';
$status = '';
//error message
$fName_er = $type_er = $dob_er = $gen_er = $marital_er = $contry_er = $city_er = $zip_er =
  $number_er = $email_er = $occup_er = $kin_er = $pass_er = $confirm_er =
  $user_er = $curr_er = $date_er = $file_er = '';
...
if (isset($_POST['register'])) {
  ...
  if ($uploadOk == 0) {
    ...
    if (
      empty($file_er) && empty($fName_er) && empty($type_er)
      && empty($gen_er) && empty($marital_er) && empty($contry_er) && empty($city_er)
      && empty($zip_er) && empty($number_er) && empty($email_er) && empty($occup_er)
      && empty($kin_er) && empty($pass_er)
      && empty($confirm_er) && empty($user_er) && empty($curr_er) && empty($date_er)
    ) {
      if (move_uploaded_file($_FILES['fileUpload']['tmp_name'], $upload_dir)) {
        ...
        $pofile_Pic = "NULL";
        $userStatus = "INACTIVE";
        if ($stmt = mysqli_prepare($conn, "INSERT INTO users(reg_Ref,Names,acc_Type,day,month,year,gender,marital,country,city,zip,ssn,phone,email,work,kin,Sav_Acc_No,Check_Acc_No,currency,photo,role)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")) {
          ...
          if (mysqli_affected_rows($conn) > 0) {
            ...
            $stmt1 = mysqli_query($conn, "INSERT INTO OnBanking(user_ref,username,password,question,answer,image,status,role,state,valid_user,reg_date)VALUES('$reg_Ref', '$user', '$pass','NULL', 'NULL', '$pofile_Pic', '$userStatus', '$role', '$state', '$checkUserOnlinebank', '$date')");
            if ($stmt1) {
              ...
              if ($stmt2) {
                ...
                $credit = 0.00;
                $tranStatus = "OPENED";
                $stmt3 = $conn->prepare("INSERT INTO real_acc(user_ref,cBal,sBal,loanBal,visaBal,creditBal,status) VALUES (?,?,?,?,?,?,?)");
                $stmt3->bind_param('sssssss', $reg_Ref, $cBal, $sBal, $loan, $visa, $credit, $tranStatus);
                $stmt3->execute();
                if ($stmt3) {
                  ...
                  if ($stmt4) {
                    ...
                    //sendMail($fName, $email, $check_Acc,$sav_Acc,$user);
